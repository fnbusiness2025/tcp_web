<script>
	...
	let observerOptions = {
		threshold: [0.1, 0.9], // Trigger when entering and leaving
	...
	onMount(() => {
		...
		const observer = new IntersectionObserver((entries) => {
			entries.forEach(entry => {
				if (entry.isIntersecting && entry.intersectionRatio > 0.1) {
					...
					// For staggered animations, add visible to children with delay
					if (entry.target.classList.contains('stagger-children')) {
						const children = entry.target.querySelectorAll('.stagger-item');
						children.forEach((child, index) => {
							setTimeout(() => {
								child.classList.add('visible');
								child.classList.remove('exiting');
							}, index * 100);
						});
					}
					...
					// For staggered animations, remove visible from children
					if (entry.target.classList.contains('stagger-children')) {
						const children = entry.target.querySelectorAll('.stagger-item');
		...
		const animatedElements = document.querySelectorAll('.animate-on-scroll, .fade-in-up, .slide-in-left, .slide-in-right, .scale-in, .stagger-children');
...
<svelte:head>
	<title>TCP Malawi - Trusted Property Opportunities in Malawi</title>
	<meta name="description" content="TCP Malawi - Find trusted property opportunities in Malawi. Discover land and real estate with confidence, transparency, and clarity." />
</svelte:head>
...
<main>
	...
	<section id="listings" class="featured">
		<div class="container">
			<div class="featured-header animate-on-scroll">
				<h2>Featured Properties</h2>
			</div>
			<div class="properties-grid stagger-children">
				<div class="property-card card stagger-item">
	...
	<section class="trust">
		<div class="container">
			<div class="trust-content animate-on-scroll">
				...
				<div class="trust-icons stagger-children">
...
<style>
	...
	.property-card.stagger-item:hover {
		transform: translateY(-8px) scale(1.02);
		box-shadow: 0 20px 40px rgba(0, 0, 0, 0.15);
	}

	.trust-item.stagger-item:hover {
		transform: translateY(-2px);
	}

	.trust-item.stagger-item:hover .trust-icon {
		background: var(--tcp-primary-dark);
		transform: scale(1.1);
	}
